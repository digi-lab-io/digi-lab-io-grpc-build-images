...
import {Book, BookId, BookMethodsController, BookMethodsControllerMethods, Books, Empty} from "./generated/book";
...
export class AppController implements BookMethodsController {
    ...
    deleteBook(request: BookId, metadata?: Metadata): Observable<Empty> {
        this.bookShelf.books = this.bookShelf.books.filter(book => book.id !== request.id);
        return from([{}])
    }

    editBook(request: Book, metadata?: Metadata): Observable<Book> {
        this.bookShelf.books = this.bookShelf.books
            .map((book) => book.id === request.id ? request : book);
        return from([request]);
    }
    ...
    getBook(request: BookId, metadata?: Metadata): Observable<Book> {
        return from([this.bookShelf.books.find(book => book.id === request.id)!]);
    }
}
